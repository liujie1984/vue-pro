<style lang="less">
    ...
    .community-one-creatat{height: 12px;line-height: 12px;font-size: 9px;color: #9a9a9a;}

    .community-one-likecomment{float: right;height: 50px;vertical-align: top;margin: 0 10px 0 0;}
    .likecomment-icon{height: 12px;width: 12px;margin: 19px 4px 0 4px;display: inline-block;vertical-align: top;}
    .likecomment-count{font-size: 12px; color: #9a9a9a;height: 16px;line-height: 16px;margin: 17px 0 0 0;display: inline-block;vertical-align: top;}
</style>
<template>
    <div class="community-one">
        ...
        <div class="community-one-content">
            <div class="community-one-left" v-link="{name:'person-other',params: { id: communitys.owner.user_id}}">
                ...
                <div class="community-one-info">
                    <p class="community-one-nickname">{{communitys.owner.nickname}}</p>
                    <p class="community-one-creatat">{{communitys.pub_date}}</p>
                </div>
            </div>
            <div class="community-one-likecomment">
                <span class="community-one-like" v-on:click="dealLike">
                    <img class="likecomment-icon" v-bind:src="likedImgSrc">
                    <span class="likecomment-count">{{communitys.likes_count}}</span>
                </span>
                <span class="community-one-comment" v-on:click="dealComment">
                    <img class="likecomment-icon" src="http://7xqamv.com2.z0.glb.qiniucdn.com/icon-comments.png">
                    <span class="likecomment-count">{{communitys.comments_count}}</span>
                </span>
            </div>
        </div>
    </div>
